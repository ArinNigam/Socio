...
function isValidInstagramUrl(InstagramUrl) {
  const regex = /^https?:\/\/(www\.)?instagram\.com\/[a-zA-Z0-9_]+\/?$/;
  return regex.test(InstagramUrl) || !InstagramUrl.length;
}
...
const UserWidget = (props) => {
  ...
  const [InstagramValue, setInstagramValue] = useState(props.user.instagramUrl);
  ...
  const addProfiles = async (e) => {
    ...
    const response = await axios.patch(
      `${process.env.REACT_APP_BACKEND_URL}/users/${props.user._id}/update/socialprofile`,
      {
        ...
        InstagramURL:
          update === "Instagram" ? InstagramValue : props.user.instagramUrl,
      },
      {
        headers: {
    ...
    if (updatedUser) {
      ...
      else if (update === "instagram") setShowInstagram(false);
      setClicked(false);
    ...
    <WidgetWrapper>
      ...
      >
        <FlexBetween gap="1rem">
          <UserImage image={props.user.profilePhoto} onClick={() => navigate(`/profile/${props.user._id}`)} />
          <Box>
            <Typography
              variant="h4"
              color={dark}
        ...
        <ManageAccounts onClick={() => navigate(`/update/${props.user._id}`)} />
      </FlexBetween>
      ...
      <Box p="1rem 0">
        ...
        <FlexBetween gap="1rem" mb="0.5rem">
          {!showInstagram && (
            <>
              <FlexBetween gap="1rem">
                <img src={InstagramImg} alt="Instagram" height="25px" width="25px" />
                <Box>
          ...
          {showInstagram && (
            <FlexBetween gap="1rem">
              ...
              <form onSubmit={addProfiles}>
                <TextField
                  label="Enter your Instagram url"
                  onChange={(e) => setInstagramValue(e.target.value.trimStart())}
                  value={InstagramValue}
                  name="Instagram"
                  error={!isValidInstagramUrl(InstagramValue)}
                  helperText={
                    !isValidInstagramUrl(InstagramValue)
                      ? "Enter valid Instagram url"
                      : !InstagramValue.length && props.user.instagramUrl
                      ? "Wanna remove your Instagram handle? Add empty field"
                      : ""
                  }
                ...
                <Button
                  ...
                  disabled={clicked || !isValidInstagramUrl(InstagramValue)}
                  onClick={() => setUpdate("Instagram")}
                >
                  ADD
                </Button>
              </form>

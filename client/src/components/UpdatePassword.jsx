...
const UpdatePassword = (props) => {
  ...
  const handleFormSubmit = async (values, onSubmitProps) => {
    ...
    const savedUserResponse = await axios.patch(
    ...
    const newUser = await savedUserResponse.data;
  ...
  const sendOtp = async (values, onSubmitProps) => {
    ...
    const newUserWithOtp = await otpResponse.data;
  ...
  const verifyOtp = async (values, onSubmitProps) => {
    ...
    const verified = await otpResponse.data;
    if (verified.error) {
      setClicked(false);
      return;
    }
      ...
      >
        {!props.forgot && (
          <Typography fontWeight="500" variant="h5" sx={{ mb: "1.5rem" }}>
            Welcome to Socio, update your password!
          </Typography>
        )}
        <Formik
